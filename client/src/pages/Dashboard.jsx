...
import AIChatModal from '../chatbot/AIChatModal'
  ...
  return (
    <div>
      ...
      <div className="d-flex flex-wrap gap-2">
        ...
        <button className="btn btn-outline-primary" onClick={() => navigate('/subjects')}>Explore Subjects</button>
      </div>
      ...
      <div className="card dashboard-card mt-4">
        <div className="card-body p-4">
          <div className="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
            <div>
              <h4 className="mb-1">Scholarships &amp; Opportunities</h4>
              <p className="text-muted mb-0">Opportunities currently available for Grade {user.grade || 'your class'}.</p>
            </div>
          ...
          {!scholarshipsLoading && !scholarshipsError && scholarships.length > 0 && (
            <div className="row g-3">
              {scholarships.slice(0, 3).map((scholarship) => (
                <div className="col-12 col-lg-4" key={scholarship._id}>
                  <div className="border rounded p-3 h-100">
                    <h6 className="fw-bold">{scholarship.title}</h6>
                    <p className="small text-muted mb-2">{scholarship.description}</p>
                    <p className="small mb-0"><strong>Deadline:</strong> {new Date(scholarship.applicationDeadline).toLocaleDateString()}</p>
                    {scholarship.applicationLink && <a className="small d-inline-block mt-2" href={scholarship.applicationLink} target="_blank" rel="noreferrer">Learn more</a>}
                  </div>
                </div>
              ))}
            </div>
